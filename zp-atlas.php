...
Plugin URI: https://n3rds.work/piestingtal_source/zodiacpress-erweiterungen/#zodiacpressatlas
Description: Deine eigene Atlas-Datenbank für ZodiacPress anstelle von GeoNames.org
Version: 1.1.2
Author: DerN3rd
...
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=zp-atlas', 
	__FILE__, 
	'zp-atlas' 
);
...
if ( ! defined( 'ZPATLAS_VERSION' ) ) {
	define( 'ZPATLAS_VERSION', '1.1.2' );// @todo update
}
...
function zpa_string( $id = '' ) {
	$strings = array(
		'active'		=> __( 'Aktiv', 'zp-atlas' ),
		'unzip' => __( 'allCountries.zip entpacken...', 'zp-atlas' ),
		'creating'		=> __( 'Tabellenschlüssel erstellen ...', 'zp-atlas' ),
		'failed_keys'	=> __( 'Fehler beim Erstellen der Tabellenschlüssel.:', 'zp-atlas' ),
		'inserting'		=> __( 'Stadtdaten in Datenbank einfügen...', 'zp-atlas' ),
		'installing'	=> __( 'Installation...', 'zp-atlas' ),
		'installing_notice' => __( 'Der Atlas wird im Hintergrund installiert. Das wird ein paar Minuten dauern.', 'zp-atlas' )
	);
	return $strings[ $id ];
}
/**
...
function zpa_admin_scripts() {
	...
	wp_localize_script( 'zp-atlas-install', 'zpAtlasStrings',
		array(
			'adminurl'		=> admin_url(),
			'checkStatus'	=> __( 'Überprüfe den Status.', 'zp-atlas' ),
			'unzip'			=> zpa_string('unzip'),
			'creatingKeys'	=> zpa_string( 'creating' ),
			'dismiss'		=> __( 'Ignoriere die Nachricht.', 'zp-atlas' ),
			...
			'statusHeading'	=> __( 'ZodiacPress-Statusmeldung', 'zp-atlas' )
		)
	);
