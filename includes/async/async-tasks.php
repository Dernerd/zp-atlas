	...
	if ( ! $temp_dir ) {
		...
		if ( file_exists( $temp_dir . $filename ) && filesize( $temp_dir . $filename ) >= zpa_gn_filesize($filename) ) {
			...
			if ( ! copy( $url, $temp_dir . $filename ) ) {
				...
				$error = __( 'Die Datei allCountries.zip konnte nicht heruntergeladen werden.', 'zp-atlas' );
...
add_action( 'wp_async_zp_atlas_insert_db', function () {
	...
	if ( ! ZPAtlas_DB::table_exists() ) {
		$error = __( 'FEHLER: Die Tabelle zp_atlas existiert nicht', 'zp-atlas' );
	...
	if ( ZPAtlas_DB::row_count() > 3000000 ) {
		...
		if ( true === $insert ) {
			...
			$error = __( 'Fehler beim Einfügen von Stadtdaten in die Datenbanktabelle.', 'zp-atlas' );
		}
	}
