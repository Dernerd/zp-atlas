...
function zpatlas_receive_heartbeat( $response, $data ) {
	...
	if ( get_transient( 'zp_atlas_ready_once' ) ) {
		...
		$response['zpatlas_status_notice'] = __( 'Die Installation des Atlas ist abgeschlossen. ZodiacPress Atlas ist gebrauchsfertig.', 'zp-atlas' );
		...
		$response['zpatlas_status_db'] = array(
			...
			'key'	=> ZPAtlas_DB::key_exists( 'PRIMARY' ) ? __( 'in Ordnung', 'zp-atlas' ) : __( 'fehlt', 'zp-atlas' ),
			'index'	=> ZPAtlas_DB::key_exists( 'ix_name_country' ) ? __( 'in Ordnung', 'zp-atlas' ) : __( 'fehlt', 'zp-atlas' ),
		);
	} else {
